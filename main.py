...
def insert_sampling(aqi, tvoc, eco2, mode):
    ...
    cur.execute(
        "INSERT INTO air (date, aqi, tvoc, eco2, mode) values (?, ?, ?, ?, ?)",
        (now, aqi, tvoc, eco2, mode),
    )
    con.commit()
    con.close()
...
def main():
    ...
    while True:
        ...
        if sample != False:
            insert_sampling(
                sreader.aqi_value, sreader.tvoc, sreader.eco2_value, sreader.operation
            )
            log(
                f"Sampled AQI:{str(sreader.aqi_value)} TVOC:{str(sreader.tvoc)}ppb eCO2:{str(sreader.eco2_value)}ppm"
            )
        else:
